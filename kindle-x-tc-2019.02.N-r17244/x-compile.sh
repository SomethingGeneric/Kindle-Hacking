...
#
# $Id: x-compile.sh 17244 2020-05-11 03:48:52Z NiLuJe $
#
# kate: syntax bash;
#
##
...
echo ""
cd ..
tar -xvJf /usr/portage/distfiles/util-linux-2.35.1.tar.xz
cd util-linux-2.35.1
update_title_info
patch -p1 < /usr/portage/sys-apps/util-linux/files/util-linux-2.35.1-include_sys_types-header.patch
patch -p1 < /usr/portage/sys-apps/util-linux/files/util-linux-2.35.1-cleanup-pidfd-include.patch
patch -p1 < /usr/portage/sys-apps/util-linux/files/util-linux-2.35.1-hwclock-glibc-2.31.patch
autoreconf -fi
...
if [[ "${KINDLE_TC}" == "KOBO" ]] ; then
	patch -p1 < ${SVN_ROOT}/Configs/trunk/Kindle/Misc/openssh-8.2p1-kobo-enforce-ash.patch
fi
...
tar xvJf /usr/portage/distfiles/ImageMagick-6.9.11-11.tar.xz
cd ImageMagick-6.9.11-11
update_title_info
...
echo ""
cd ..
tar -I pigz -xvf /usr/portage/distfiles/tmux-3.1b.tar.gz
cd tmux-3.1b
update_title_info
...
update_title_info
tar -xvJf /usr/portage/distfiles/libpcre2-10.34-patchset-01.tar.xz
# Gentoo Patches...
for patchfile in patches/* ; do
	# Try to detect if we need p0 or p1...
	if grep -q 'diff --git' "${patchfile}" ; then
...
echo ""
cd ..
tar -I pigz -xvf /usr/portage/distfiles/less-560.tar.gz
cd less-560
update_title_info
